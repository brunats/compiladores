...
void inserirInstLit(int x, char *literal){
	printf("\n%s\n\n",literal);
	vetInstrucoes[contadorInstrucoes].inst = x;
	vetInstrucoes[contadorInstrucoes].label = -1;
	vetInstrucoes[contadorInstrucoes].op1 = -1;
	vetInstrucoes[contadorInstrucoes].op2 = -1;
	vetInstrucoes[contadorInstrucoes].str = malloc(strlen(literal)*sizeof(char));
	strcpy(vetInstrucoes[contadorInstrucoes].str,literal);
	contadorInstrucoes++;
}
...
void adicionarInstrucao(int i, int n){
	switch(i){
		...
		case ISUB:
			...
			break;
		case IINVOKE:
			inserirVetorInst(IINVOKE,-1,-1,-1);
			break;
		case IGETSTATIC:
			inserirVetorInst(IGETSTATIC,-1,-1,-1);
			break;
		case JINVOKE:
			inserirVetorInst(JINVOKE,-1,-1,-1);
			break;
	}
}
/*
